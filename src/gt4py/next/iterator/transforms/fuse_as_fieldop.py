...
class FuseAsFieldOp(
    ...
    def transform_fuse_as_fieldop(self, node: itir.Node, **kwargs):
        if cpm.is_applied_as_fieldop(node):
            ...
            # Block kolor-range-expanding fusions: if the inner arg has a Kolor domain that
            # differs from the outer's domain, inlining would evaluate the inner's kolor-specific
            # shift over a wider domain.  For example, inlining
            # as_fieldop(shift(Kolor,-2), [2,3))(dwdz) into an outer with domain [0,3) causes
            # DaCe to evaluate shift(Kolor,-2) at kolor=0 → reads dwdz at kolor=-2 → GPU OOB.
            # Only inline when inner kolor range EQUALS outer kolor range (safe: same extent).
            import os as _fuse_os
            _debug_kolor = _fuse_os.environ.get("STRUCTURED_DEBUG_SHAPES", "0") == "1"

            def _kolor_interval(n: itir.Expr, label: str = ""):
                # Try multiple ways to find a cartesian_domain (some passes canonicalize domains
                # into node.annex.domain). Accept both applied-as-fieldop nodes and nodes that
                # carry a domain in their annex.
                dom = None
                if cpm.is_applied_as_fieldop(n) and len(n.fun.args) > 1:
                    dom = n.fun.args[1]
                # fallback: some transformations move the domain into annex.domain
                if dom is None and hasattr(n, "annex"):
                    dom = getattr(n.annex, "domain", None)

                if dom is None:
                    if _debug_kolor:
                        print(f"[KOLOR_DBG] {label}: no domain found (checked fun.args and annex)")
                    return None

                if not cpm.is_call_to(dom, "cartesian_domain"):
                    if _debug_kolor:
                        print(f"[KOLOR_DBG] {label}: dom not cartesian_domain, dom type={type(dom).__name__}, dom={str(dom)[:80]}")
                    return None

                axis_names_found: list[str] = []
                for r in dom.args:
                    if not (cpm.is_call_to(r, "named_range") and len(r.args) == 3):
                        continue
                    axis_name = getattr(r.args[0], "value", None)
                    axis_names_found.append(str(axis_name))
                    if axis_name == "Kolor":
                        lo, hi = r.args[1], r.args[2]

                        def _int_val(x):
                            # Accept OffsetLiteral and Literal (ConstantFolding may convert)
                            if isinstance(x, itir.OffsetLiteral) and isinstance(x.value, int):
                                return x.value
                            if isinstance(x, itir.Literal):
                                try:
                                    return int(x.value)
                        ...
                        lo_v, hi_v = _int_val(lo), _int_val(hi)
                        if lo_v is not None and hi_v is not None:
                            if _debug_kolor and lo_v < 0:
                                # Negative Kolor lo is suspicious — trace full domain
                                print(f"[KOLOR_DBG_NEG] {label}: Kolor=[{lo_v},{hi_v}) lo<0! full_dom={str(dom)[:200]}")
                            return (lo_v, hi_v)
                        if _debug_kolor:
                            print(f"[KOLOR_DBG] {label}: Kolor found but bad lo/hi types: lo={type(lo).__name__}({getattr(lo,'value',lo)}), hi={type(hi).__name__}({getattr(hi,'value',hi)})")
                        return None

                if _debug_kolor:
                    print(f"[KOLOR_DBG] {label}: no Kolor named_range in domain, axes={axis_names_found}")
                return None

            outer_kolor = _kolor_interval(node, "outer")
            # Block fusion when the inner has a different (narrower) kolor domain than the outer.
            # Gate removed: outer_kolor=None means no domain or full-range — also unsafe to fuse
            # kolor-specific inner nodes into it.
            for i, (arg, elig) in enumerate(zip(args, eligible_els)):
                if elig and cpm.is_applied_as_fieldop(arg):
                    inner_kolor = _kolor_interval(arg, f"inner[{i}]")
                    # Block when inner has a kolor restriction that outer doesn't share
                    # or when inner kolor bounds look out-of-range (negative lo or too-large hi).
                    if inner_kolor is not None:
                        lo, hi = inner_kolor
                        # conservative bound check: negative low is always unsafe
                        if (lo is not None and lo < 0) or (hi is not None and hi > 3):
                            if _debug_kolor:
                                print(f"[KOLOR_DBG_BLOCK] blocking fusion for arg[{i}] due to OOB kolor interval [{lo},{hi})")
                            eligible_els[i] = False
                            continue
                    if inner_kolor is not None and inner_kolor != outer_kolor:
                        # Inner has kolor restriction that outer doesn't share → block
                        eligible_els[i] = False

            if _debug_kolor:
                def _kolor_range_str(n, lbl=""):
                    iv = _kolor_interval(n, lbl)
                    return f"[{iv[0]},{iv[1]})" if iv is not None else "?"
                if any(eligible_els):
                    outer_k = _kolor_range_str(node, "outer_str")
                    for i, (arg, elig) in enumerate(zip(args, eligible_els)):
                        if elig and cpm.is_applied_as_fieldop(arg):
                            print(f"[FUSE] outer_kolor={outer_k} inlining arg[{i}] inner_kolor={_kolor_range_str(arg, f'inner_str[{i}]')}")
            if any(eligible_els):
                return self.visit(
                    fuse_as_fieldop(
                        node,
