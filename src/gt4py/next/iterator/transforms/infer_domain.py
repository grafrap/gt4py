...
def _infer_concat_where(
    ...
    **kwargs: Unpack[InferenceOptions],
    ...
    def _symbolic_domain_from_cond(cond_expr: itir.Expr) -> domain_utils.SymbolicDomain:
        if cpm.is_call_to(cond_expr, "or_"):
            # Flatten nested or_: structured backend can generate or_(A, or_(B, C)).
            stack = list(cond_expr.args)
            domains: list[domain_utils.SymbolicDomain] = []
            while stack:
                sub_cond = stack.pop()
                if cpm.is_call_to(sub_cond, "or_"):
                    stack.extend(sub_cond.args)
                else:
                    domains.append(_symbolic_domain_from_cond(sub_cond))
            return domain_utils.domain_union(*domains)

        # _build_edge_validity_masked_expr produces and_(cartesian_domain(Kolor),
        # cartesian_domain(IDim), cartesian_domain(JDim)). SymbolicDomain.from_expr
        # only handles a single cartesian_domain, so merge the sub-domains.
        if cpm.is_call_to(cond_expr, "and_"):
            ...
            while stack:
                ...
                if cpm.is_call_to(sub_cond, "and_"):
                    ...
                    sub_domain = _symbolic_domain_from_cond(sub_cond)
                    merged_ranges.update(sub_domain.ranges)
                    grid_type = sub_domain.grid_type
            return domain_utils.SymbolicDomain(grid_type, merged_ranges)

        return domain_utils.SymbolicDomain.from_expr(cond_expr)

    symbolic_cond = _symbolic_domain_from_cond(cond)
