...
pp_2d = raw_vertices.reshape((ny + 1, nx + 1, 1))  # Shape: [nx+1, ny+1]
...
backend_name = os.environ.get("SIMPLE_STRUCTURED_BACKEND", "gtfn").strip().lower()
if backend_name == "dace":
    selected_backend = run_dace_cpu
elif backend_name == "gtfn":
    selected_backend = gtfn_cpu
else:
    raise ValueError(
        f"Unsupported SIMPLE_STRUCTURED_BACKEND='{backend_name}'. Use 'gtfn' or 'dace'."
    )
